...
// Process incoming POST data from hardware_survey.php
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['response'])) {
    ...
    // Determine the next page dynamically (should be results.php)
    $nextPage = getNextSurveyPage($_SESSION['selected_people'], $_SESSION['selected_tools'], 'analog');
...
// Fetch selected analog tools
$selectedAnalogDetails = [];
foreach ($tools as $toolId) {
    $stmt = $pdo->prepare("SELECT tool_id, tool_name, tool_type FROM tools WHERE tool_id = :tool_id AND tool_type = 'analog'");
    $stmt->execute(['tool_id' => $toolId]);
    if ($tool = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $selectedAnalogDetails[] = $tool;
    }
}

// If no analog tools are selected, skip to the next page immediately
if (empty($selectedAnalogDetails)) {
    $nextPage = getNextSurveyPage($_SESSION['selected_people'], $_SESSION['selected_tools'], 'analog');
    header("Location: $nextPage");
    exit();
}

// Fetch questions for 'analog' category
$questionsStmt = $pdo->query("SELECT question_id, question_text FROM questions WHERE category = 'analog'");
$analogQuestions = $questionsStmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Survey MVP - Analog Questions</title>
...
<body>
    <div class="survey-wrapper">
        <div class="survey-sidebar">
            ...
            <p class="sidebar-text">Evaluate your analog tools. Your feedback shapes insights.</p>
        ...
        <div class="survey-main">
            <div class="section-header">
                <h2 class="section-title">Analog Survey for <?php echo htmlspecialchars("{$user['first_name']} {$user['last_name']}"); ?></h2>
            </div>

            <?php if (!empty($selectedAnalogDetails)): ?>
                    ...
                    <div class="table-container">
                        <table class="survey-table">
                            <thead>
                                <tr>
                                    <th>Questions</th>
                                    <?php foreach ($selectedAnalogDetails as $tool): ?>
                                        <th><?php echo htmlspecialchars($tool['tool_name']); ?></th>
                                    <?php endforeach; ?>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (empty($analogQuestions)): ?>
                                    <tr>
                                        <td colspan="<?php echo count($selectedAnalogDetails) + 1; ?>" style="text-align: center; color: #6B7280; padding: 2rem;">
                                            No questions available for the 'analog' category.
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($analogQuestions as $question): ?>
                                        <?php
                                        ...
                                        <tr>
                                            ...
                                            <?php if (empty($optionsList)): ?>
                                                <td colspan="<?php echo count($selectedAnalogDetails); ?>" style="text-align: center; color: #6B7280;">
            ...
            <?php else: ?>
                <p style="text-align: center; color: #6B7280; padding: 2rem;">
                    No analog tools selected. Proceeding to the next step...
                </p>
            <?php endif; ?>
        </div>
    </div>
